...
# 1. uploadDocument helper
if 'uploadDocument' not in content:
    content = content.replace('// ─── Add Technician Modal', """const uploadDocument = async (file: File | null, id: string, name: string) => {
...
content = re.sub(
    r'const \[certifications, setCertifications\] = useState<string\[\]>\(\[\]\);.*?const \[saveError, setSaveError\] = useState\(""\);',
    r'''const [docs, setDocs] = useState({
...
# 4. AddTechModal handleSave
content = re.sub(
    r'const handleSave = async \(\) => \{.*?setTimeout\(\(\) => \{[^{}]*?onAdd\(newTech\);.*?\}, 900\);\s*\};',
...
# 5. AddTechModal UI insertion
content = content.replace('{/* Estado */}', f'{docsUI}\n\n            {{/* Estado */}}', 1)

# 6. AddTechModal Save button
content = content.replace('{saved ? "¡Guardado!" : "Guardar Chofer"}', '{uploading ? "Subiendo..." : saved ? "¡Guardado!" : "Guardar Chofer"}', 1)
content = content.replace('onClick={handleSave}', 'onClick={handleSave}\n                disabled={uploading}', 1)

# 7. EditTechModal states
content = re.sub(
    r'const \[saving, setSaving\] = useState\(false\);.*?const \[saveError, setSaveError\] = useState\(""\);',
...
# 8. EditTechModal handleSave
content = re.sub(
    r'const handleSave = async \(\) => \{.*?setTimeout\(\(\) => \{.*?\}, 800\);\s*setSaving\(false\);\s*\};',
    r'''const handleSave = async () => {
